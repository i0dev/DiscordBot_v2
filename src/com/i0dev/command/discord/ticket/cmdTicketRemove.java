...
import com.i0dev.object.Ticket;
...
public class cmdTicketRemove extends ListenerAdapter {

    private final String Identifier = "Ticket Remove";
    private final List<String> COMMAND_ALIASES = getConfig.get().getStringList("commands.ticketRemove.aliases");
    private final boolean REQUIRE_PERMISSIONS = getConfig.get().getBoolean("commands.ticketRemove.requirePermission");
    private final boolean REQUIRE_LITE_PERMISSIONS = getConfig.get().getBoolean("commands.ticketRemove.permissionLiteMode");
    private final String MESSAGE_CONTENT = getConfig.get().getString("commands.ticketRemove.messageContent");
    private final String MESSAGE_FORMAT = getConfig.get().getString("commands.ticketRemove.format");
    private final boolean COMMAND_ENABLED = getConfig.get().getBoolean("commands.ticketRemove.enabled");
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (MessageAliases.isMessageACommand(e.getMessage(), COMMAND_ALIASES)) {
            ...
            e.getChannel().sendMessage(EmbedFactory.get().createSimpleEmbed(Placeholders.convert(MESSAGE_CONTENT.replace("{userRemoved}", MentionedUser.getAsTag()), e.getAuthor())).build()).queue();
