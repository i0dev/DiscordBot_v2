...
public class cmdInvites extends ListenerAdapter {


    private final String Identifier = "Invites";
    private final List<String> COMMAND_ALIASES = getConfig.get().getStringList("commands.invites.aliases");
    private final boolean REQUIRE_PERMISSIONS = getConfig.get().getBoolean("commands.invites.requirePermission");
    private final boolean REQUIRE_LITE_PERMISSIONS = getConfig.get().getBoolean("commands.invites.permissionLiteMode");
    private final String messageContent = getConfig.get().getString("commands.invites.messageContent");
    private final String MESSAGE_FORMAT = getConfig.get().getString("commands.invites.format");
    private final boolean COMMAND_ENABLED = getConfig.get().getBoolean("commands.invites.enabled");
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (MessageAliases.isMessageACommand(e.getMessage(), COMMAND_ALIASES)) {
            ...
            e.getChannel().sendMessage(EmbedFactory.get().createSimpleEmbed(Placeholders.convert(messageContent, MentionedUser)).build()).queue();
