...
public class cmdStaffClear extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (MessageAliases.isMessageACommand(e.getMessage(), COMMAND_ALIASES)) {
            ...
            e.getGuild().getTextChannelById(staffMovementsChannelID).sendMessage(EmbedFactory.get().createSimpleEmbed(Placeholders.convert(MESSAGE_CONTENT.replace("{senderTag}", e.getAuthor().getAsTag()), MentionedUser)).build()).queue();
        }
    }
}
