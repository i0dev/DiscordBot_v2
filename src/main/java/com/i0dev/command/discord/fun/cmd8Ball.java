...
public class cmd8Ball extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (MessageAliases.isMessageACommand(e.getMessage(), COMMAND_ALIASES)) {
            ...
            e.getChannel().sendMessage(EmbedFactory.get().createSimpleEmbed(Placeholders.convert(MESSAGE_CONTENT.replace("{question}", FormatUtil.remainingArgFormatter(message, 1)).replace("{answer}", Ans + ""), e.getAuthor())).build()).queue();
        }
    }
}
