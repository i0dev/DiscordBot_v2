...
public class CmdProfile extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (MessageAliases.isMessageACommand(e.getMessage(), COMMAND_ALIASES)) {
            ...
            //String invitedById = InviteMatcher.get().getNewJoinObject(MentionedUser).get("invitedBy").toString();
           // User invitedBy = InternalJDA.get().getJda().getUserById(invitedById);
            ...
            desc.append("**Total Tickets Closed:** ").append("`" + TicketTop.get().getUserInviteCount(MentionedUser) + "`").append("\n");
         //  desc.append("**Invited By:** ").append("`" + (invitedById == null ? "No Log" : invitedBy.getAsTag()) + "`").append("\n");
