package com.i0dev.command.discord.reactionroles;
    ...
    public void onPrivateMessageReceived(PrivateMessageReceivedEvent e) {
        ...
        if (CurrentQuestion == Questions.size() - 1) {
            if (!messageContent.equalsIgnoreCase("submit")) {
                e.getChannel().sendMessage(EmbedFactory.get().createSimpleEmbedNoThumbnail("Unknown message, to submit your reaction panel, type `submit`, to cancel type `" + GlobalConfig.GENERAL_BOT_PREFIX + "cancel`").build()).queue();
                return;
            }
