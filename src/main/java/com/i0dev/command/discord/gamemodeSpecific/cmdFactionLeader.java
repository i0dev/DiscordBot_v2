...
public class cmdFactionLeader extends ListenerAdapter {

    private final String Identifier = "Faction Leader";
    private final List<String> COMMAND_ALIASES = getConfig.get().getStringList("commands.factionLeader.aliases");
    private final boolean REQUIRE_PERMISSIONS = getConfig.get().getBoolean("commands.factionLeader.requirePermission");
    private final boolean REQUIRE_LITE_PERMISSIONS = getConfig.get().getBoolean("commands.factionLeader.permissionLiteMode");
    private final String MESSAGE_CONTENT = getConfig.get().getString("commands.factionLeader.messageContent");
    private final String MESSAGE_FORMAT = getConfig.get().getString("commands.factionLeader.format");
    private final boolean COMMAND_ENABLED = getConfig.get().getBoolean("commands.factionLeader.enabled");

    private final String factionLeaderRole = getConfig.get().getString("roles.factionLeaderRoleID");
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (MessageAliases.isMessageACommand(e.getMessage(), COMMAND_ALIASES)) {
            ...
            RoleQueue.addToQueue(MentionedUser.getId(), factionLeaderRole);
