    ...
    public void onPrivateMessageReceived(PrivateMessageReceivedEvent e) {
        ...
        if (CurrentQuestion == Questions.size() - 1) {
            if (!messageContent.equalsIgnoreCase("submit")) {
                e.getChannel().sendMessage(EmbedFactory.get().createSimpleEmbedNoThumbnail("Unknown message, to create your giveaway, type `submit`, to cancel type `" + conf.GENERAL_BOT_PREFIX + "cancel`").build()).queue();
                return;
            }
