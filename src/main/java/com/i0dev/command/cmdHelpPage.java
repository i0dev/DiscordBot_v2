...
public class cmdHelpPage extends ListenerAdapter {
    ...
    private String page3() {
        ...
        desc.append("{enabled}``{cmd} <Message ID>`` - *Reroll a giveaway*\n".replace("{enabled}", (getConfig.get().getBoolean("commands.giveawayReroll.enabled") + "").replace("true", enabledEmoji).replace("false", disabledEmoji)).replace("{cmd}", conf.GENERAL_BOT_PREFIX + getConfig.get().getStringList("commands.giveawayReroll.aliases").get(0)));
    ...
    private String page4() {
        ...
        desc.append("{enabled}``{cmd} <amount> [note]`` - *Create a Tebex GiftCard with the specified amount!*\n".replace("{enabled}", (getConfig.get().getBoolean("commands.tebexGiftcardCreate.enabled") + "").replace("true", enabledEmoji).replace("false", disabledEmoji)).replace("{cmd}", conf.GENERAL_BOT_PREFIX + getConfig.get().getStringList("commands.tebexGiftcardCreate.aliases").get(0)));
        desc.append("\n**Extended Other Commands:**\n");
        desc.append("{enabled}``{cmd} <Server IP>`` - *Get general information about the Minecraft server.*\n".replace("{enabled}", (getConfig.get().getBoolean("commands.MCServerInfo.enabled") + "").replace("true", enabledEmoji).replace("false", disabledEmoji)).replace("{cmd}", conf.GENERAL_BOT_PREFIX + getConfig.get().getStringList("commands.MCServerInfo.aliases").get(0)));
